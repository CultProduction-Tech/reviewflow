...
router.post('/', (req, res) => {
  try {
    ...
    db.prepare('INSERT INTO sections (id, name, project_id, sort_order) VALUES (?, ?, ?, ?)')
      .run(uuidv4(), 'Все файлы', projectId, 0);
    ...
    if (reviewer_groups && reviewer_groups.length > 0) {
      reviewer_groups.forEach((group, index) => {
        db.prepare('INSERT INTO reviewer_groups (id, name, project_id, sort_order) VALUES (?, ?, ?, ?)')
          .run(uuidv4(), group.name || `Группа рецензентов ${index + 1}`, projectId, index);
      });
    } else {
      db.prepare('INSERT INTO reviewer_groups (id, name, project_id, sort_order) VALUES (?, ?, ?, ?)')
        .run(uuidv4(), 'Группа рецензентов 1', projectId, 0);
    }
    ...
    res.status(500).json({ error: 'Ошибка создания проекта: ' + err.message });
  }
});
